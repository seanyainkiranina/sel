...
class Tester:
    ...
    def action_start(self) -> object:
        ...
        self.save_cursor.execute(
            "insert into detail_unit_tests(master_id,start,step_id,run_id,p_id,individual) values(?,CURRENT_TIMESTAMP,?,?,?,?); select scope_identity() as id",
            [self.step.master_id, self.step.current_step, self.my_uuid, pid, self.step.user_name])
